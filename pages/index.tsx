...
const theme = localData.game.themes[confirmPick(localData.game.themes.length) - 1]
...
const Home = () => {
  ...
  const { images } = theme
	...
	const handleClick = ({ currentTarget: { id } }: SyntheticEvent) => {
		document.getElementById(id)!.style.transition = "transform 100ms"
		...
		<div className="text-warning p-2" style={{ maxWidth: 400 }}>
			...
			<div>
				<div className="d-flex justify-content-between">
					<h3 className="bg-info p-1 rounded">Pick a card</h3>
					{chance! > 0 ? (
						<h3
							className={`${
						...
						<h3 className="bg-danger px-2 rounded">Game Over!</h3>
					)}
				</div>
				<div
					style={{
